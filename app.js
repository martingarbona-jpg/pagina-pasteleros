...
function abrirModalNovedades(item, version) {
  ...
  const linkTexto = escaparHtmlNovedades(item.linkTexto || "Ver mas");
      ...
      function render() {
        ...
        carrusel.innerHTML = `
          <article class="novedades-card">
            ${imagen ? `
              <div class="novedades-card__media">
                <img src="${imagen}" alt="${titulo}" onerror="this.closest('.novedades-card__media').remove()">
              </div>
            ` : ""}
            <div class="novedades-card__body">
              <h3>${titulo}</h3>
              ${descripcion ? `<p>${descripcion}</p>` : ""}
              ${link ? `<a class="btn novedades-card__link" href="${escaparHtmlNovedades(link)}" target="_blank" rel="noopener">${linkTexto}</a>` : ""}
            </div>
            ${hayVarios ? `
